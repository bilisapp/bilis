...
- **`database/clickhouse/SCHEMA.md` is the source of truth** for the `otel_logs`, `otel_traces` and `trace_summary` tables: pinned collector tag, exact DDL, rules R1–R12. Column names and types belong to the OTel exporter (R1); `ORDER BY`, `PARTITION BY`, `TTL`, indexes and `ProjectId` are ours. Read it before touching the DDL or any query against the table.
